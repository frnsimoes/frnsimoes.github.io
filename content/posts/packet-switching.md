...
Routers just read the header, check the destination, and forward. There are no billing timers or "port 5 connected to port 12". Intelligence moved to the edges, where hosts handle reliability, retransmission, ordering, and congestion. 
...
In the 1990s, connecting to the internet meant making a phone call[^2]. Your modem called the ISP's modem pool, and the telephone network established a circuit - your house -> local exchange -> ISP. A dedicated 64 kbps path, just like a voice call, with the phone company's switches tracking it as a connection and billing timers running.

Your computer sent IP packets to the modem, the modem modulated them into [analog signals](https://www.eetimes.com/an-introduction-to-the-v-90-56k-modem/) over the phone line, and the ISP's modem demodulated back to digital and forwarded to the internet.

Here in Brazil we'd connect after midnight or on weekends so we'd only pay for one tick. You'd plan your internet time.

This hybrid existed because the last mile was telephone lines - copper twisted pair. ISPs had to use what was there. Cable modems did something similar with coaxial cable, and fiber eventually replaced both, but the pattern was always the same: reuse existing physical infrastructure.
...
[^1]: Voice digitization: 8-bit samples at 8 kHz = 64 kbps. https://dsp.stackexchange.com/questions/22107/why-is-telephone-audio-sampled-at-8-khz

[^2]: If you're feeling technical, I recommend this blog post about the sound of dial-up: https://www.windytan.com/2012/11/the-sound-of-dialup-pictured.html
