...
While doing some routine upgrades, I restarted hundreds of pods of a Nodejs websocket application. Every pod came back at ~250 MiB of memory, but one of them soon started to be hammered by some automation tool we have, and memory spike to 500+ MiB, and stayed there.

This pattern wasn't new. I've been seeing this for months. Well, today I decided to understand what's happening, and my first bet is the usual culprit: V8's garbage collection. 
...
252 MiB gone. So the V8 theory was right. But was glibc arena fragmentation also real, or had I been hunting the wrong thing entirely? I properly tested both this time. The first try was with --max-old-space-size=256 only. This caps V8's old generation heap at 256 MiB, forcing it to actually collect garbage. Pod was hammered again, memory spiked to 619 MiB during the burst, then dropped to 350 MiB. Better. But 350 is still above the ~250 MiB baseline, so I set malloc arena to 2 again. This time it dropped to around the baseline and stayed there. 
