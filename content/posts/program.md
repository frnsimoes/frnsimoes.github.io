+++
date = 2025-10-21
title = "Breaking an executable file"
labels = ["post"]
+++
...
If you try to execute it, you will get "Exec format error", so `excve` get `-1` and raises ENOEXEC. This happens because, with no program headers, the kernel cannot tell which bytes are code or where they go in memory, so there is nothing for it to load.  
...
Excecute it. No ENOEXEC because the program headers are intact and the kernel loads the file happily, `excve` reutnrs 0 as well. But when the kernel tries to jump to the entry point and start running, there is no code - so we get a sefgault ("killed by SIGSSEGV"). Ok, so my first intuition is that a program is something the kernel can map into memory and start executing. If it has a segment to load, it's happy; an address to jump to, also happy. 

 `ls` is dynamically linked: and when we run a dynamically-linked program, the kernel does not jump to its entry point, it jumps to the *interpreter* - the dynamic linker - and lets it load the libraries and eventually call the code. The interpretr is just another field in the file, a path string, which you can see with readelf:
...
`ENOEXEC` again - the kernel has no handler that recognizes this file. Note that the problem isn't the extension `zerowing`, the issue is that the first line of the file is garbage to the kernel becase `ZeroWing` means nothing. 
...
Ok, so wrapping all of this so far: a simple binary with its program headers or its entry point zeroed: not a program. A binary whose interpreted was swapoed: it runs, but runs the swapped interpreter. A text file that does whatever its first line says, and dies if that line points back at itself. A `ZeroWing` file that was garbage until I registered a handler.
...
`readelf` reads it without any issues: a valid 64-bit executable with four `LOAD` segments. We have a valid entry point too. But when I try to run it, `ENOEXEC` again. `e_machine` is part of the ABI: rules for how code and the processor work together - instruction encoding, registers, how a syscall is made, etc. This also means x86-64 instrumention are not valid to an ARM core, for obvious reasons. So the kernel checks the architecture first, before it maps anything. If the CPU cannot run the code, everything else is irrelevante.

---

### June 2026 update

Since I wrote this article, I've read mutliple interesting materials exploring files and programs. Two of them are simple great, and I highly recommend if this got you interested and you want to dig deeper.
