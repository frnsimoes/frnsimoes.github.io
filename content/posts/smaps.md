...
A few weeks ago [I profiled a Node.js server](/pmem/) with recurring memory spikes. I found out dirty pages, allocator behavior and memory that did not return to the OS. To build a cleaner mental model, I stripped the problem down to something smaller: python3 -m http.server. 
...
The kernel exposes everything it knows about a process's memory in [/proc/pid/maps](https://man7.org/linux/man-pages/man5/proc_pid_maps.5.html). Each line is a region of virtual memory that the kernel allocated for this process.
...
Anonymous memory is expensive. A file-backed page can be evicted from RAM and reloaded from disk because the file is still there. An anonymous page has no backing file. If the kernel needs to reclaim it, the only option is swap. If there's no swap, the page stays in RAM until the process frees it or dies.
...
Despite the differences between the kernel and libc point of view, you can actually see the allocators perspective of resident bytes and used bytes with malloc_stats(3).
...
Shared with every other process that uses libc, libm, libz. When the kernel loads libc's .text segment, it doesn't give the pages directly to the process. It puts them in the page cache, which is a cache of file contents, and points the process's page table at those cached pages. If another python3 starts, its page table points to the same physical frames. In summary, there is one copy of libc's code in RAM, and it is used by everyone.
