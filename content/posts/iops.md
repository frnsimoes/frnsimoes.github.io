+++
date = 2026-02-08
title = "The layers between select and disk"
labels = ["draft"]
subjects = ["database", "production"]
+++

Follow-up: I later reproduced this on hardware I control and measured every cache layer - shared buffers, page cache, NVMe, EBS - in microseconds. [What an 8 KB read costs](/8kb-read).


We had a Postgres incident earlier this week. A bunch of Heroku timeouts and queries running for more than 30 minutes. IOPS was hitting the pvosioned limit. I kept thinking: what actually happens when Postgres reads from disk?
...
Ours was set to 4GB, which is the default recommendation. The [Postgres docs](https://www.postgresql.org/docs/current/runtime-config-resource.html) suggest 25% of total RAM, and there's a reason for that.
