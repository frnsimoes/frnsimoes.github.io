...
**about shell session and process groups**
...
A session has an environment. For example, if you want to know every variable available in your shell session, you can check it with `declare -x`. 
...
When we source a file (or use the `.` command, which is equivalent), we're telling the shell to read and execute commands from that file in the current shell environment, not in a new process. This is crucial because: 1. The commands are executed in the current shell's memory space (similar to builtins). 2. Any changes to environment variables, functions, or aliases persist in the current session.
...
By sourcing, we're essentially saying "read these commands and execute them right here, in this shell session." That's why all the changes stick around - they're modifying the current session's environment, not creating a new one.
