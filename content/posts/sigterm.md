...
A clear contradiction. A few possible explanations.

Processes don't stay in one state. A process doing disk I/O goes D, then S, then D, then S again as individual reads complete and new ones start. Load average samples every 5 seconds, so it could catch them in D at sampling time even if they spend most of their time in S. The ones I killed might have been in an S window when the signal arrived.
