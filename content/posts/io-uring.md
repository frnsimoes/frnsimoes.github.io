...
Without readahead, io_uring issues only one entry at a time. The problem is the lack of concurrency: each read waits for the previous one. The application knows that "hey, at this time, I need page 100", which means: Turso submits a read SQE for page 100, then waits for it. The scan continues. Now Turso needs page 101, so it submits a new read SQE that page.[^sequential]
...
[^sequential]: The example above assumes sequential physical readahead. [Alex Miller](transactional.blog) pointed out why that's naive for a database, quoted with permission:
    "Readahead as implemented by cached IO is a purely physical readahead, and so it’s a bet that the table/index is clustered — that the pages logically sort in the same order they’re physically sorted in.  Btree pages can be in any order, and so that is just a bet. It’s not unusual for scan operators to be able to hint as to if they’re going to be doing sequential reads (table scan) or random reads (index join), and let the storage apply the hint both for caching reasons (so large scans don’t empty the working set from memory) and for IO reads (don’t do readahead on random reads). I think there’s some systems out there which can do full table scans by sweeping the file and picking out leaf pages in unsorted order, but SQLite is not one of them.  It follows a btree walk, so the right place to put readahead is into that btree walk so that it can fetch the actual pages you’ll read next rather than just a random guess."
